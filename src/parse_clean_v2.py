...
INTERIM = Path("data/interim/daily_sales_parsed_v2.csv")
PROCESSED = Path("data/processed/daily_sales_fact_v2.csv")
DQ = Path("data/processed/data_quality_report_v2.csv")
...
def parse_revenue(text: str):
    """
    Handles:
    - R27,401 / R 23,596 / R12450 / 11848
    - 9.1k / 14.8k (with or without R)
    - R0 / 0
    ...
    # Normalize for quick patterns
    t = text.lower().replace(" ", "")

    # k format (e.g., 9.1k, r9.1k)
...
def resolve_sale_date(claimed_date, msg_ts):
    ...
    - if message sent next-day morning (06:00–11:00) and claimed_date == msg_date - 1 → keep claimed_date
    - if claimed date differs from msg date by > 1 day → use msg date
    """
...
def main():
    ...
    # Duplicate detection (rep_key handles missing rep_id)
    ...
    # Save interim (message-level)
    df.to_csv(INTERIM, index=False)

    # Fact table (rep/store/day)
